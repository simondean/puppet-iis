...
When /^puppet applies the manifest$/ do
  ...
  begin
    ...
    system({ 'BUNDLE_BIN_PATH' => nil, 'BUNDLE_GEMFILE' => nil, 'GEM_HOME' => nil, 'GEM_PATH' => nil, 'RUBYLIB' => nil, 'RUBYOPT' => nil }, "cmd", "/c", File.join(ENV['ProgramFiles(x86)'], 'Puppet Labs/Puppet/bin/puppet.bat'), 'apply', '--debug', '--detailed-exitcodes', '--modulepath', './modules', manifest_file.path)
